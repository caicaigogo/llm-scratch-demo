...
class SFTDataCollator(object):
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for x in batch:
            ...
            # target_mask 报错，改成了attention_mask，可能是因为forward里没target_mask吧，但感觉也不应该。
            target_mask = x['attention_mask']
        ...
        # 这里做了修改，0 应该改成 -100
        labels = torch.where(target_mask_batch == 1, input_ids_batch, 0)
        inputs = {
            'input_ids': input_ids_batch,
            'attention_mask': attention_mask_batch,
            'labels': labels
