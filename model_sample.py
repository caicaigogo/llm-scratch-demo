...
class TextGenerator:
    def __init__(self, 
        ...
        self.model = LLaMAForCausalLM(ModelConfig(hidden_size=512, num_hidden_layers=2))  # 实例化 Transformer 模型
            ...
            with self.ctx:  # 进入自动混合精度的上下文（如果是 GPU 并使用 float16 时）
                for k in range(num_samples):  # 循环生成指定数量的样本
                    y = self.model.generate(
                        x, self.tokenizer.eos_token_id, max_new_tokens, temperature=temperature, top_k=top_k
                    )  # 生成文本
...
if __name__ == "__main__":
    ...
    generator = TextGenerator(checkpoint='./base_model_215M/pretrain_512_2_6144.pth')  # 初始化生成器
    ...
    generator = TextGenerator(checkpoint='./sft_model_215M/512_2_6144.pth')  # 初始化生成器
